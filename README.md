...
### 装置有messure，knob这些类属性，messure是指观测值，如湿度，温度等信息，knob是配置信息，如内存资源，cpu占用率等等机器的硬件信息。装置会根据控制器的不同控制指令结合messure，然后模拟出在这些指令下knob的变化。每个装置有唯一的时空位置，能够记录下在任何时空位置下机器硬件信息的变化。
